...
class PropertyValuationTool(BaseTool):
    ...
    def run_sync(self, transaction_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for property_data in properties:
                property_id = property_data.get("id", "") or property_data.get(
                    "property_id", ""
                )
                if not property_id:
                ...
                building_value = self._get_numeric_value(
                    property_data,
                    [
                        "building_value",
                        "building_values",
                ...
                square_footage = self._get_numeric_value(
                    property_data,
                    [
                        "square_footage",
                        "square_feet",
                ...
                # Track other values
                content_value = self._get_numeric_value(
                    property_data,
                    [
                        "contents",
                        "content_value",
                ...
                business_income_value = self._get_numeric_value(
                    property_data,
                    [
                        "business_income",
                        "bi",
                ...
                construction_type = self._extract_value(
                    property_data,
                    [
                        "construction",
                        "construction_type",
                ...
                year_built = self._get_numeric_value(
                    property_data,
                    [
                        "year_built",
                        "year",
                ...
                sprinklered = self._extract_boolean(
                    property_data,
                    ["sprinklered", "sprinkler", "has_sprinklers", "fire_protection"],
                )

                # Extract roof information
                roof_age = self._get_numeric_value(
                    property_data, ["roof_age", "roof_year", "roof"]
                )
                ...
                if square_footage <= 0:
                    calculated_value = (
                        building_value  # Default to reported value if no square footage
                    )
                    ...
                    matched_construction = self._find_closest_match(
                        construction_type, self._construction_costs.keys()
                    )
                    matched_occupancy = self._find_closest_match(
                        occupancy, self._occupancy_factors.keys()
                    )
                    ...
                    base_cost_per_sqft = self._construction_costs.get(
                        matched_construction, 125
                    )

                    # Apply state/regional cost multiplier
                    regional_multiplier = (
                        self._state_multipliers.get(state, 1.0) if state else 1.0
                    )

                    # Apply occupancy factor
                    occupancy_factor = self._occupancy_factors.get(
                        matched_occupancy, 1.0
                    )
                ...
                if building_value > 0:
                    variance_percentage = (
                        (calculated_value - building_value) / building_value
                    ) * 100
                else:
                ...
                risk_score = self._calculate_risk_score(
                    risk_flags, variance_percentage, building_age, roof_age
                )
                ...
                if include_citations:
                    citations = self._generate_citations(
                        matched_construction, state, matched_occupancy, building_age
                    )
                ...
                property_valuation = {
                    ...
                    "details": {
                        ...
                        "construction_type": (
                            ...
                            if "matched_construction" in locals()
                            else "Unknown"
                        ),
                        "year_built": int(year_built) if year_built > 0 else None,
                        "building_age": int(
                            building_age
                        ),  # Now building_age is always defined
                        "occupancy": (
                            matched_occupancy
                            if "matched_occupancy" in locals()
                            else "Unknown"
                        ),
                        "state": state,
            ...
            if reported_total > 0:
                total_variance_percentage = (
                    (calculated_total - reported_total) / reported_total
                ) * 100
            ...
            analysis_summary = {
                ...
                "avg_property_value": (
                    float(reported_total / len(property_valuations))
                    if len(property_valuations) > 0
                    else 0
                ),
                "valuation_quality": self._get_valuation_quality(
                    total_variance_percentage,
                    len(all_anomalies),
                    len(property_valuations),
                ),
    ...
    def _extract_properties_from_submission(
        ...
        print(
            f"Extracting properties from submission data with keys: {list(submission_data.keys())}"
        )

        # Step 1: Extract standard properties from "Property" section
        if "Property" in submission_data and isinstance(
            submission_data["Property"], list
        ):
            print(
                f"Found {len(submission_data['Property'])} properties in 'Property' section"
            )
            for item in submission_data["Property"]:
                if isinstance(item, dict):
                    ...
                    if "standard_facts" in item and isinstance(
                        item["standard_facts"], dict
                    ):
                    ...
                    if "limits" in item and isinstance(item["limits"], dict):
                        ...
                        if "100_pct_coverage_limits" in item["limits"] and isinstance(
                            item["limits"]["100_pct_coverage_limits"], dict
                        ):
                            for key, value in item["limits"][
                        ...
                        if "100_pct_limit" in item["limits"]:
                            if (
                                isinstance(item["limits"]["100_pct_limit"], dict)
                                and "value" in item["limits"]["100_pct_limit"]
                            ):
                                property_data["building_value"] = item["limits"][
                                    "100_pct_limit"
                                ]["value"]
                            else:
                                property_data["building_value"] = item["limits"][
                    ...
                    if "building_details" in item and isinstance(
                        item["building_details"], dict
                    ):
                    ...
                    if "location_doc_id" in property_data:
                        ...
                        property_data["_location_id"] = (
                            location_id  # Special matching key
                        )

                    if "location_address" in property_data:
                        property_data["_address"] = property_data[
                            "location_address"
        ...
        if "Advanced Property" in submission_data and isinstance(
            submission_data["Advanced Property"], list
        ):
            print(
                f"Found {len(submission_data['Advanced Property'])} properties in 'Advanced Property' section"
            )
            for item in submission_data["Advanced Property"]:
                if isinstance(item, dict):
                    ...
                    if "advanced_facts" in item and isinstance(
                        item["advanced_facts"], dict
                    ):
                    ...
                    if "location_id" in property_data:
                        ...
                        match = re.search(
                            r"([0-9]+|[A-Z][0-9][A-Z][0-9]+)", location_id
                        )
                        if match:
        ...
        print(
            f"Starting property merge process with {len(standard_properties)} standard and {len(advanced_properties)} advanced properties"
        )
        ...
        for adv_prop in advanced_properties:
            ...
            if "_location_doc_id" in adv_prop:
                ...
                if key in property_map:
                    ...
                    print(
                        f"Matched advanced property by doc_id: {doc_id} -> property {matched_index+1}"
                    )
            ...
            if matched_index is None and location_id:
                ...
                for i, std_prop in enumerate(merged_properties):
                    ...
                    if std_location_id and std_location_id == location_id:
                        matched_index = i
                        print(
                            f"Matched advanced property by location_id: {location_id} -> property {i+1}"
                        )
                        break
            ...
            if matched_index is None:
                ...
                if address:
                    ...
                    if key in property_map:
                        matched_index = property_map[key]
                        print(
                            f"Matched advanced property by address: {address} -> property {matched_index+1}"
                        )
                ...
                for i, merged_prop in enumerate(merged_properties):
                    ...
                    if std_address and adv_address:
                        ...
                        if len(common_parts) > 1:  # If they share multiple words
                            matched_index = i
                            print(
                                f"Matched advanced property by similar address: {adv_address} -> property {i+1}"
                            )
                            break

            # Try to match by numeric property order if addresses or locations aren't available
            if matched_index is None and len(advanced_properties) == len(
                standard_properties
            ):
                # Try to find a property that hasn't been matched with advanced data yet
                for i, merged_prop in enumerate(merged_properties):
                    if (
                        merged_prop.get("source") == "standard"
                        and "total_building_area_sqft" not in merged_prop
                    ):
                        matched_index = i
                        print(
                            f"Matched advanced property by position: advanced {adv_prop.get('location_id', '')} -> property {i+1}"
                        )
                        break
            ...
            if matched_index is not None:
                ...
                for key, value in adv_prop.items():
                    if (
                        key not in merged_properties[matched_index]
                        or not merged_properties[matched_index][key]
                    ):
                ...
                print(
                    f"Added new property from advanced data: {adv_prop.get('location_id', '')}"
                )
        ...
        for i, prop in enumerate(merged_properties):
            ...
            zip_code = (
                prop.get("zip", "")
                or prop.get("zip_code", "")
                or prop.get("postal_code", "")
                or prop.get("location_postal_code", "")
            )
            ...
            state = self._extract_value(prop, ["state", "state_code", "location_state"])
            ...
            business_income_value = self._get_numeric_value(
                ...
                [
                    ...
                    "business_interruption",
                    "time_element",
                    "coverage_business_income",
                    "coverage_business_interruption",
                ],
            )
                ...
                for field in [
                    ...
                    if field in [
                        ...
                        merged[field] = (
                            values[0]
                            if values
                            else (0 if field in ["year_built", "roof_age"] else "")
                        )
    ...
    def _extract_address(self, property_data: Dict[str, Any]) -> str:
        ...
        for field in address_fields:
            if field in property_data and property_data[field]:
                ...
                if (
                    isinstance(property_data[field], dict)
                    and "value" in property_data[field]
                ):
        ...
        state = self._extract_value(
            property_data, ["state", "state_code", "location_state"]
        )
        if state:
            address_components.append(str(state))

        # Zip
        zip_code = self._extract_value(
            property_data, ["zip", "zip_code", "postal_code", "location_postal_code"]
        )
        if zip_code:
            ...
            if state_field in property_data:
                ...
                if value and isinstance(value, str):
                    ...
                    for state_name, code in state_codes.items():
                        if state_name.startswith(value_upper) or value_upper.startswith(
                            state_name
                        ):
                            return code
    ...
    def _identify_risk_flags(
        ...
        if building_value > 0 and calculated_value > 0:
            ...
            if building_value < (
                calculated_value * (self._risk_thresholds["underinsurance"] / 100)
            ):
                risk_flags.append("POTENTIAL_UNDERINSURANCE")

            if building_value > (
                calculated_value * (self._risk_thresholds["overinsurance"] / 100)
            ):
        ...
        if building_value > 0 and content_value > 0:
            ...
            if content_ratio > self._risk_thresholds["content_ratio_high"]:
                ...
                content_ratio < self._risk_thresholds["content_ratio_low"]
                and building_value > 1000000
            ):
    ...
    def _calculate_risk_score(
        ...
        if (
            "MISSING_SQUARE_FOOTAGE" in risk_flags
            or "MISSING_BUILDING_VALUE" in risk_flags
        ):
        ...
        if building_age > 0:
            age_score = min(
                building_age / 100 * self._risk_score_weights["building_age"],
    ...
    def _generate_recommendations(
        ...
        if "POTENTIAL_UNDERINSURANCE" in risk_flags:
            ...
            recommendations.append(
                f"Consider increasing building value by approximately ${deficit:,.0f} ({abs(variance_percentage):.1f}%) for adequate coverage"
            )
            recommendations.append(
                "Schedule a professional appraisal to validate replacement cost estimates"
            )

        if "POTENTIAL_OVERINSURANCE" in risk_flags:
            excess = building_value - calculated_value
            recommendations.append(
                f"Property may be overinsured by approximately ${excess:,.0f} ({abs(variance_percentage):.1f}%); verify reported value"
            )

        if "MISSING_SQUARE_FOOTAGE" in risk_flags:
            recommendations.append(
                "Verify square footage - current value is missing or zero"
            )

        if "MISSING_BUILDING_VALUE" in risk_flags:
            recommendations.append(
                f"Building value is missing or zero; estimated replacement cost is ${calculated_value:,.0f}"
            )

        if "OLDER_CONSTRUCTION" in risk_flags:
            recommendations.append(
                f"Consider requesting engineering inspection due to age of building ({building_age} years old)"
            )
            recommendations.append(
                "Verify building code upgrades have been factored into valuation"
            )

        if "AGING_ROOF" in risk_flags:
            recommendations.append(
                f"Roof age ({roof_age} years) exceeds typical service life; request roof inspection"
            )

        if "HIGH_CONTENT_RATIO" in risk_flags:
            recommendations.append(
                "Content value is unusually high compared to building value; verify accuracy"
            )

        if "LOW_CONTENT_RATIO" in risk_flags:
            recommendations.append(
                "Content value may be underreported for a building of this value"
            )

        return recommendations

    def _generate_citations(
        self, construction_type: str, state: str, occupancy: str, building_age: int
    ) -> List[Dict[str, Any]]:
    ...
    def _get_valuation_quality(
        self, variance_percentage: float, anomaly_count: int, total_count: int
    ) -> str:
